use {crate::error::ArtifactIdError, std::io::BufRead};
...
pub(crate) trait ForEachBufFill: BufRead {
    ...
    fn for_each_buf_fill(&mut self, f: impl FnMut(&[u8])) -> Result<usize, ArtifactIdError>;
}

impl<R: BufRead> ForEachBufFill for R {
    fn for_each_buf_fill(&mut self, mut f: impl FnMut(&[u8])) -> Result<usize, ArtifactIdError> {
